...
scriptExcecuteStatus() {
	...
	then
        	echo "[ $(date '+%d-%m-%Y %H:%M:%S') ] WARNING: Since $(basename ${0}) is Already Running Exiting the new Instance."
...
checkCACHEAlertNotification() {
        ...
        echo "CACHE_ALERT_NOTIFICATION :: $eventId $eventName $eventType $cacheAlertOCuuraceCnt [ $cacheAlertThresholdCnt ]"
}
...
sendTrap() {
        ...
        then
                ...
                echo "CACHING THE EVENT :: $eventName - $eventId - $eventType - $errorMessage [ $cacheAlertOCuuraceCnt ]"
                rm -f "${statusFile}"
        fi
}
...
getStatus() {
	...
	then
		...
		then
			...
			sendTrap "${eventId}" "${eventName}" "${errorMessage}" "${eventType}"
		fi
		rm -f "${statusFile}"
	fi
}
...
checkMysqlConnectivityStatus() {
	...
	then
		...
		eventType="LOW"
		getStatus "$statusFile" "$eventId" "$eventName" "$errorMessage" "$eventType"
	fi
}
...
checkMysqlLockMGNR () {
	...
	then
		...
		#mysqlLockCnt=1
		find . -type f -name ".${eventId}_${eventName}_${eventType}" -mmin+8 rm -f {} \;
		...
		then
			...
			if [ -f .${eventId}_${eventName}_${eventType} ]
			then 
				rm -f .${eventId}_${eventName}_${eventType}
				echo "[CRITICAL_EVENT] ${warning}"
				warning="${warning} "\n"$(mysql -uroot -proot -h127.0.0.1 -P"${mysqlPort}" -Ae "SHOW FULL PROCESSLIST;" | grep -v grep | egrep -i "LOCK|Sending data")"
				errorMessage=${warning}
				echo HIGH > "$statusFile"
				eventType="HIGH"
				sendTrap "$eventId" "$eventName" "$errorMessage" "$eventType"
			else 
				echo ${warning} > .${eventId}_${eventName}_${eventType}
			fi
		else
			eventId="STS_E_1018"
...
checkMysqlReplicationStatus () {
        slaveStatusSQL="show slave status"
        if (($#==2))
        then
                tmpChannel=${2}
                channel=" for channel '"${2}"'"
                slaveStatusSQL=${slaveStatusSQL}${channel}
        fi
        mysqlPort="${1}"
        slaveStatusSQL="${slaveStatusSQL}\G"
        checkMysqlConnectivityStatus "${mysqlPort}"
        if ((connectStatus > 0))
        then
                eventId="STS_E_1016"
                eventName="REPLICATIONSTATUS"
                eventType="EVENT"
                statusFile=".ReplicationStatus${mysqlPort}${tmpChannel}"
                ...
                dbPort=${mysqlPort}
                timeStamp="[ $(date '+%d-%m-%Y %H:%M:%S') ]"
                mysql -u${dbUserName} -p${dbPasswd} -h${dbIPAdd} -P"${dbPort}" -A -e "${slaveStatusSQL};" > "${statusFile}Info"
                slaveIOStatus=$(< "${statusFile}Info" grep Slave_IO_Running: | awk -F':' '{print $2}' | sed -e 's/ //g')
                slaveSQLStatus=$(< "${statusFile}Info" grep Slave_SQL_Running: | awk -F':' '{print $2}' | sed -e 's/ //g')
                #lastErrMsg=$(< "${statusFile}Info" grep Last_Error | awk -F'Last_Error: ' '{print $2}')
                #lastSqlErr=$(< "${statusFile}Info" grep Last_SQL_Error | awk -F'Last_SQL_Error: ' '{print $2}')
                ...
                slaveIOStatus=$(echo "${slaveIOStatus}" | tr "[:lower:]" "[:upper:]")
                slaveSQLStatus=$(echo "${slaveSQLStatus}" | tr "[:lower:]" "[:upper:]")
		echo "[ $(date '+%d-%m-%Y %H:%M:%S') ] ${eventName} :: ${dbIPAdd}:${dbPort} :: ${tmpChannel} [ IO_STATUS :: ${slaveIOStatus}, SQL_STATUS :: ${slaveSQLStatus} ]"
                if [ "${slaveIOStatus}" = "NO" ] || [ "${slaveSQLStatus}" = "NO" ]
                then
                        errorMessage="REPLICATION Breaks at ${timeStamp} in DB SERVER having IP ${ipAddress} ($(/bin/hostname)) with Port ${dbPort} and${channel}."
                        echo "[CRITICAL_EVENT] ${errorMessage}"
                        echo HIGH > "$statusFile"
                        eventType="HIGH"
                        sendTrap "$eventId" "$eventName" "$errorMessage" "$eventType"        
                        ...
                        eventId="STS_E_1017"
                        errorMessage="Replication Came Up on ${ipAddress} ($(/bin/hostname)) with Port ${dbPort} and${channel}."
                        eventType="LOW"
                        getStatus "$statusFile" "$eventId" "$eventName" "$errorMessage" "$eventType"
                fi
                rm -f "${statusFile}Info"
...
systemCPULoadAverageThresholdValue=24
appCPUUsgThresholdValue=800
...
if (((fiveCpuLoad > systemCPULoadAverageThresholdValue)) && ((fifCpuLoad > systemCPULoadAverageThresholdValue))) || (((fiveCpuLoad > systemCPULoadAverageThresholdValue)) && ((oneCpuLoad > systemCPULoadAverageThresholdValue)))
then
...
checkMysqlReplicationStatus 3306 "channel-m2-to-m1"
#checkMysqlReplicationStatus 3306 "channel-d1-to-m1"
#checkMysqlReplicationStatus 3306 "channel-d2-to-m1"
checkMysqlReplicationStatus 3307 "channel-m2-to-m1"
#checkMysqlReplicationStatus 3307 "channel-d1-to-m1"
#checkMysqlReplicationStatus 3307 "channel-d2-to-m1"
###checkMysqlReplicationStatus 3308 "channel-m2-to-m1"
###checkMysqlReplicationStatus 3308 "channel-d1-to-m1"
###checkMysqlReplicationStatus 3308 "channel-d2-to-m1"


#checkMysqlLockMGNR 3306
checkMysqlConnectionMGNR 3306 "/etc/my.cnf"

checkMysqlConnectivityStatus 3308
checkMysqlConnectivityStatus 3309
